...
class ControllerExtensionPaymentOpenpayBanks extends Controller {
    ...
    public function index() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            ...
            $this->model_setting_setting->editSetting('payment_openpay_banks', $setting);
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('extension/payment/openpay_banks', 'user_token=' . $this->session->data['user_token'], true),
        );

        $data['action'] = $this->url->link('extension/payment/openpay_banks', 'user_token=' . $this->session->data['user_token'], true);
        $data['cancel'] = $this->url->link('marketplace/extension', 'user_token=' . $this->session->data['user_token'], true);

        $data['payment_openpay_banks_test_merchant_id'] = $this->fillSetting('payment_openpay_banks_test_merchant_id');
        $data['payment_openpay_banks_live_merchant_id'] = $this->fillSetting('payment_openpay_banks_live_merchant_id');
        $data['payment_openpay_banks_test_public_key'] = $this->fillSetting('payment_openpay_banks_test_public_key');
        $data['payment_openpay_banks_test_secret_key'] = $this->fillSetting('payment_openpay_banks_test_secret_key');
        ...
        $data['payment_openpay_banks_status'] = $this->fillSetting('payment_openpay_banks_status');
        $data['payment_openpay_banks_total'] = $this->fillSetting('payment_openpay_banks_total');
        $data['payment_openpay_banks_sort_order'] = $this->fillSetting('payment_openpay_banks_sort_order');        
        $data['payment_openpay_banks_geo_zone_id'] = $this->fillSetting('payment_openpay_banks_geo_zone_id');

        $data['header'] = $this->load->controller('common/header');
        $data['column_left'] = $this->load->controller('common/column_left');
        $data['footer'] = $this->load->controller('common/footer');
        
        $this->response->setOutput($this->load->view('extension/payment/openpay_banks', $data));
    }
    ...
    protected function validate() {
        ...
        if ($this->request->post['payment_openpay_banks_test_mode']) {            
            ...
            if (empty($this->request->post['payment_openpay_banks_test_secret_key'])) {
            ...
            if (empty($this->request->post['payment_openpay_banks_live_merchant_id'])) {
                $this->error['live_merchant_id'] = $this->language->get('error_live_merchant_id');
            }
            
            if (empty($this->request->post['payment_openpay_banks_live_secret_key'])) {
        ...
        if (!isset($this->request->post['payment_openpay_banks_total']) || (float) $this->request->post['payment_openpay_banks_total'] < (float) $min_total) {
    ...
    private function createWebhook($mode){        
        if(!$this->config->get('payment_openpay_banks_'.$mode.'_webhook')){                        
