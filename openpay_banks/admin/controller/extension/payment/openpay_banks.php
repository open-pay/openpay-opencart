...
class ControllerExtensionPaymentOpenpayBanks extends OpenpayBanksController {
    ...
    public function index() {
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('extension/payment/openpay_banks', 'token=' . $this->session->data['token'], 'SSL'),
        );

        $data['action'] = $this->url->link('extension/payment/openpay_banks', 'token=' . $this->session->data['token'], 'SSL');
        ...
        $this->response->setOutput($this->load->view('extension/payment/openpay_banks.tpl', $data));
    }
    ...
    protected function createWebhook($mode){
        ...
        if(!$this->config->get('openpay_bank_'.$mode.'_webhook')){
            ...
            $webhook_data = array(
                'url' => $protocol.$_SERVER['HTTP_HOST'].'/index.php?route=extension/payment/openpay_banks/webhook',
    ...
    public function install() {
        ...
        $this->model_extension_payment_openpay_banks->install();                
    }

    public function uninstall() {
        //$this->load->model('extension/payment/openpay_banks');
        //$this->model_extension_payment_openpay_banks->uninstall();
    ...
    public function orderAction() {
        if (defined('PRO_MOD') && PRO_MOD && isset($this->request->get['order_id']) && ( $charge = $this->fetchCharge($this->request->get['order_id']) )) {
            ...
            $data['txn'] = $this->model_extension_payment_openpay_banks->getTransactions(array('charge_ref' => $charge->id));
            $data['url_capture'] = HTTPS_SERVER . 'index.php?route=extension/payment/openpay_banks/jsonCapture&token=' . $this->session->data['token'] . '&order_id=' . $this->request->get['order_id'];
            $data['url_refund'] = HTTPS_SERVER . 'index.php?route=extension/payment/openpay_banks/jsonRefund&token=' . $this->session->data['token'] . '&order_id=' . $this->request->get['order_id'];

            return $this->load->view('extension/payment/openpay_banks_order.tpl', $data);
        }
    }
