...
class ControllerPaymentOpenpayBanks extends OpenpayBanksController {
    ...
    public function index() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            ...
            $mode = $this->request->post['openpay_bank_test_mode'] ? 'test' : 'live';
            
            $webhook = $this->createWebhook($mode);
            if(!$webhook->error && $webhook != false){
                $setting['openpay_bank_'.$mode.'_webhook'] = $webhook->id;
            }
            
            $setting['openpay_total_currency'] = $this->currency->getCode();
            $this->model_setting_setting->editSetting('openpay', $setting);
            $this->model_setting_setting->editSetting('openpay_banks', $setting);
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('payment/openpay_banks', 'token=' . $this->session->data['token'], 'SSL'),
        );

        $data['action'] = $this->url->link('payment/openpay_banks', 'token=' . $this->session->data['token'], 'SSL');
        $data['cancel'] = $this->url->link('extension/payment', 'token=' . $this->session->data['token'], 'SSL');

        $data['openpay_bank_test_merchant_id'] = $this->fillSetting('openpay_bank_test_merchant_id');
        $data['openpay_bank_live_merchant_id'] = $this->fillSetting('openpay_bank_live_merchant_id');
        $data['openpay_bank_test_public_key'] = $this->fillSetting('openpay_bank_test_public_key');
        $data['openpay_bank_test_secret_key'] = $this->fillSetting('openpay_bank_test_secret_key');
        $data['openpay_bank_live_public_key'] = $this->fillSetting('openpay_bank_live_public_key');
        $data['openpay_bank_live_secret_key'] = $this->fillSetting('openpay_bank_live_secret_key');
        $data['openpay_bank_deadline'] = $this->fillSetting('openpay_bank_deadline');
        $data['openpay_bank_test_mode'] = $this->fillSetting('openpay_bank_test_mode');
        $data['openpay_bank_captured_status_id'] = $this->fillSetting('openpay_bank_captured_status_id');
        $data['openpay_bank_new_status_id'] = $this->fillSetting('openpay_bank_new_status_id');
        $data['openpay_bank_title'] = $this->fillSetting('openpay_bank_title', $this->language->get('text_title'));

        $this->load->model('localisation/order_status');

        $data['order_statuses'] = $this->model_localisation_order_status->getOrderStatuses();
        $data['openpay_banks_geo_zone_id'] = $this->fillSetting('openpay_banks_geo_zone_id');
        ...
        $data['openpay_banks_status'] = $this->fillSetting('openpay_banks_status');
        $data['openpay_bank_total'] = $this->fillSetting('openpay_bank_total');
        $data['openpay_banks_sort_order'] = $this->fillSetting('openpay_banks_sort_order');
        ...
        $this->response->setOutput($this->load->view('payment/openpay_banks.tpl', $data));
    }

    protected function validate() {
        if (!$this->user->hasPermission('modify', 'payment/openpay_banks')) {
            $this->error['warning'] = $this->language->get('error_permission');
        }
        
        if ($this->request->post['openpay_bank_test_mode']) {
            
            if (empty($this->request->post['openpay_bank_test_merchant_id'])) {
                $this->error['test_merchant_id'] = $this->language->get('error_test_merchant_id');
            }
            
            if (empty($this->request->post['openpay_bank_test_secret_key'])) {
                $this->error['test_secret_key'] = $this->language->get('error_test_secret_key');
            }
            if (empty($this->request->post['openpay_bank_test_public_key'])) {
                $this->error['test_public_key'] = $this->language->get('error_test_public_key');
            }
            
            if(!$this->getMerchantInfo($this->request->post['openpay_bank_test_merchant_id'], $this->request->post['openpay_bank_test_secret_key'], $this->request->post['openpay_bank_test_mode'])){
                $this->error['test_merchant_account'] = $this->language->get('error_test_merchant_account');                
            }
            
        } else {
            
            if (empty($this->request->post['openpay_bank_live_merchant_id'])) {
                $this->error['live_merchant_id'] = $this->language->get('error_live_merchant_id');
            }
            
            if (empty($this->request->post['openpay_bank_live_secret_key'])) {
                $this->error['live_secret_key'] = $this->language->get('error_live_secret_key');
            }
            if (empty($this->request->post['openpay_bank_live_public_key'])) {
                $this->error['live_public_key'] = $this->language->get('error_live_public_key');
            }
            
            if(!$this->getMerchantInfo($this->request->post['openpay_bank_live_merchant_id'], $this->request->post['openpay_bank_live_secret_key'], $this->request->post['openpay_bank_test_mode'])){
                $this->error['live_merchant_account'] = $this->language->get('error_live_merchant_account');
            }
            
        }

        if (!isset($this->request->post['openpay_bank_total']) || (float) $this->request->post['openpay_bank_total'] < (float) MIN_TOTAL) {
    ...
    protected function createWebhook($mode){
        ...
        if(!$this->config->get('openpay_bank_'.$mode.'_webhook')){
            $protocol = $_SERVER['HTTPS'] ? 'https://' : 'http://';
            $domain = $_SERVER['HTTP_HOST'];
            $webhook_data = array(
                'url' => $protocol.$domain.'/index.php?route=payment/openpay_banks/webhook',
    ...
    public function install() {
        ...
        $this->model_payment_openpay_banks->createOrderTable();
        $this->model_payment_openpay_banks->createTransactionTable();        
        $this->model_payment_openpay_banks->createCustomerTable();
    }

    public function uninstall() {
//        if ($this->config->get('openpay_uninstall_table')) {
//            $this->load->model('payment/openpay_banks');
//            $this->model_payment_openpay_banks->deleteOrderTable();
//            $this->model_payment_openpay_banks->deleteTransactionTable();
//            $this->model_payment_openpay_banks->deleteCustomerTable();
//        }
//        if ($this->config->get('openpay_uninstall_setting')) {
//            $this->load->model('setting/setting');
...
//        }
    ...
    public function orderAction() {
        if (defined('PRO_MOD') && PRO_MOD && isset($this->request->get['order_id']) && ( $charge = $this->fetchCharge($this->request->get['order_id']) )) {
            ...
            $data['txn'] = $this->model_payment_openpay_banks->getTransactions(array('charge_ref' => $charge->id));
            $data['url_capture'] = HTTPS_SERVER . 'index.php?route=payment/openpay_banks/jsonCapture&token=' . $this->session->data['token'] . '&order_id=' . $this->request->get['order_id'];
            $data['url_refund'] = HTTPS_SERVER . 'index.php?route=payment/openpay_banks/jsonRefund&token=' . $this->session->data['token'] . '&order_id=' . $this->request->get['order_id'];

            return $this->load->view('payment/openpay_banks_order.tpl', $data);
        }
    }
