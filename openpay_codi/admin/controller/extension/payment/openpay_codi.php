...
class ControllerExtensionPaymentOpenpayCodi extends Controller {
    ...
    public function index() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            $setting = $this->model_setting_setting->getSetting('payment_openpay_codi');
            $this->merge($setting, $this->request->post, true);
            
            $mode = $this->request->post['payment_openpay_codi_mode'] ? 'test' : 'live';            
            $webhook = $this->createWebhook($mode);
            if(!$webhook->error && $webhook != false){
                $setting['payment_openpay_codi_'.$mode.'_webhook'] = $webhook->id;
            }
                        
            $this->model_setting_setting->editSetting('payment_openpay_codi', $setting);
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('extension/payment/openpay_codi', 'user_token=' . $this->session->data['user_token'], true),
        );

        $data['action'] = $this->url->link('extension/payment/openpay_codi', 'user_token=' . $this->session->data['user_token'], true);
        $data['cancel'] = $this->url->link('marketplace/extension', 'user_token=' . $this->session->data['user_token'], true);

        $data['payment_openpay_codi_test_merchant_id'] = $this->fillSetting('payment_openpay_codi_test_merchant_id');
        $data['payment_openpay_codi_live_merchant_id'] = $this->fillSetting('payment_openpay_codi_live_merchant_id');
        $data['payment_openpay_codi_test_public_key'] = $this->fillSetting('payment_openpay_codi_test_public_key');
        $data['payment_openpay_codi_test_secret_key'] = $this->fillSetting('payment_openpay_codi_test_secret_key');
        $data['payment_openpay_codi_live_public_key'] = $this->fillSetting('payment_openpay_codi_live_public_key');
        $data['payment_openpay_codi_live_secret_key'] = $this->fillSetting('payment_openpay_codi_live_secret_key');
        $data['payment_openpay_codi_mode'] = $this->fillSetting('payment_openpay_codi_mode');        
        $data['payment_openpay_codi_order_status_id'] = $this->fillSetting('payment_openpay_codi_order_status_id');
        $data['payment_openpay_codi_title'] = $this->fillSetting('payment_openpay_codi_title', $this->language->get('text_title'));
        $data['payment_openpay_codi_expiration_time'] = $this->fillSetting('payment_openpay_codi_expiration_time');
        $data['payment_openpay_codi_unit_time'] = $this->fillSetting('payment_openpay_codi_unit_time');
        
        $data['unit_time'] = array('minutes' => 'Minuto (s)', 'hours' => 'Hora (s)', 'days' => 'Dia (s)');
        $data['codi_expiration'] = $this->fillSetting('payment_openpay_codi_expiration');
        ...
        $data['payment_openpay_codi_status'] = $this->fillSetting('payment_openpay_codi_status');
        $data['payment_openpay_codi_total'] = $this->fillSetting('payment_openpay_codi_total');
        $data['payment_openpay_codi_sort_order'] = $this->fillSetting('payment_openpay_codi_sort_order');        
        $data['payment_openpay_codi_geo_zone_id'] = $this->fillSetting('payment_openpay_codi_geo_zone_id');

        $data['header'] = $this->load->controller('common/header');
        $data['column_left'] = $this->load->controller('common/column_left');
        $data['footer'] = $this->load->controller('common/footer');
        
        $this->response->setOutput($this->load->view('extension/payment/openpay_codi', $data));
    }

    protected function validate() {
        $min_total = 1;
        if (!$this->user->hasPermission('modify', 'extension/payment/openpay_codi')) {
            $this->error['warning'] = $this->language->get('error_permission');
        }
        
        if ($this->request->post['payment_openpay_codi_mode']) {            
            if (empty($this->request->post['payment_openpay_codi_test_merchant_id'])) {
                $this->error['test_merchant_id'] = $this->language->get('error_test_merchant_id');
            }
            
            if (empty($this->request->post['payment_openpay_codi_test_secret_key'])) {
                $this->error['test_secret_key'] = $this->language->get('error_test_secret_key');
            }            
            
            if(!$this->getMerchantInfo($this->request->post['payment_openpay_codi_test_merchant_id'], $this->request->post['payment_openpay_codi_test_secret_key'], $this->request->post['payment_openpay_codi_mode'])){
                $this->error['test_merchant_account'] = $this->language->get('error_test_merchant_account');                
            }
            
        } else {
            
            if (empty($this->request->post['payment_openpay_codi_live_merchant_id'])) {
                $this->error['live_merchant_id'] = $this->language->get('error_live_merchant_id');
            }
            
            if (empty($this->request->post['payment_openpay_codi_live_secret_key'])) {
                $this->error['live_secret_key'] = $this->language->get('error_live_secret_key');
            }            
            
            if(!$this->getMerchantInfo($this->request->post['payment_openpay_codi_live_merchant_id'], $this->request->post['payment_openpay_codi_live_secret_key'], $this->request->post['payment_openpay_codi_mode'])){
                $this->error['live_merchant_account'] = $this->language->get('error_live_merchant_account');
            }
            
        }

        if (!isset($this->request->post['payment_openpay_codi_total']) || (float) $this->request->post['payment_openpay_codi_total'] < (float) $min_total) {
    ...
    private function createWebhook($mode){        
        if(!$this->config->get('payment_openpay_codi_'.$mode.'_webhook')){                        
            $protocol = (isset($this->request->server['HTTPS']) && (($this->request->server['HTTPS'] == 'on') || ($this->request->server['HTTPS'] == '1'))) ? 'https://' : 'http://';            
            $webhook_data = array(
                'url' =>  $protocol.$_SERVER['HTTP_HOST'].'/index.php?route=extension/payment/openpay_codi/webhook',
    ...
    public function install() {
        $this->load->model('extension/payment/openpay_codi');
        $this->model_extension_payment_openpay_codi->install();                
    }

    public function uninstall() {
        //$this->load->model('extension/payment/openpay_codi');
        //$this->model_extension_payment_openpay_codi->uninstall();
    ...
    private function getMerchantId() {
        if ($this->config->get('payment_openpay_codi_mode')) {
            return $this->config->get('payment_openpay_codi_test_merchant_id');
        }
        return $this->config->get('payment_openpay_codi_live_merchant_id');
    }
    ...
    private function isTestMode() {
        if ($this->config->get('payment_openpay_codi_mode') == '1') {
            return true;
        } else {
            return false;
        }
    ...
    private function getSecretApiKey() {
        if ($this->config->get('payment_openpay_codi_mode')) {
            return $this->config->get('payment_openpay_codi_test_secret_key');
        }
        return $this->config->get('payment_openpay_codi_live_secret_key');
    }
