...
class ControllerExtensionPaymentOpenpayCodi extends Controller {
    ...
    public function index() {
        ...
        $data['continue'] = $this->url->link('extension/payment/openpay_codi/confirm');
        ...
        return $this->load->view('extension/payment/openpay_codi', $data); 
    }

    public function confirm() {        
        if (array_key_exists('payment_method', $this->session->data) && $this->session->data['payment_method']['code'] == 'openpay_codi') {
            ...
            if ($this->customer->isLogged()) {
                $customer = $this->model_extension_payment_openpay_codi->getCustomer($this->customer->getId());
            }
            ...
            if (array_key_exists('openpay_charge', $this->session->data)) {
                ...
                $order_info = $this->model_checkout_order->getOrder($this->session->data['order_id']);
                if ($this->currency->convert($order_info['total'], $order_info['currency_code'], $this->config->get('openpay_total_currency')) < (float) $this->config->get('payment_openpay_codi_total')) {
                ...
                if($this->config->get('payment_openpay_codi_expiration')){
                    $expiration_time = $this->config->get('payment_openpay_codi_expiration_time');
                    $unit_time = $this->config->get('payment_openpay_codi_unit_time');

                    date_default_timezone_set('America/Mexico_City');
                    $due_date = date('Y-m-d\TH:i:s', strtotime('+ '.$expiration_time.' '.$unit_time));
                ...
                if (isset($charge->error)) {
                    ...
                    $this->model_extension_payment_openpay_codi->addOrder(array(
            ...
            $data['barcode_base64'] = $charge->payment_method->barcode_base64;
            $data['due_date'] = $charge->due_date;
            ...
            $this->response->setOutput($this->load->view('extension/payment/openpay_codi_receipt', $data));                       
        }else{
    ...
    public function webhook(){
        ...
        if ($charge->method == 'bank_account') {
            if ($json->type == 'charge.succeeded' && $charge->status == 'completed') {
                ...
                $notify = true;
                $this->load->model('checkout/order');
                $this->model_checkout_order->addOrderHistory($charge->order_id, $this->config->get('payment_openpay_codi_order_status_id'), $comment, $notify);
    ...
    private function createOpenpayCustomer($customer_data, $oc_customer_id) {       
        try {            
            ...
            if ($this->customer->isLogged()) {                
                $this->model_extension_payment_openpay_codi->addCustomer(array('customer_id' => $oc_customer_id, 'openpay_customer_id' => $customer->id));                
            }
    ...
    private function createOpenpayCharge($customer, $charge_request) {
        try {                        
            ...
            $this->load->model('extension/payment/openpay_codi');
            $this->model_extension_payment_openpay_codi->addTransaction(array('type' => 'Charge creation', 'customer_ref' => $customer->id, 'charge_ref' => $charge->id, 'amount' => $charge->amount, 'status' => $charge->status));
    ...
    private function sendReceipt ($order_info, $pdf_url) {      
        ...
        $mail->setHtml($this->load->view('extension/payment/openpay_codi_mail', $data));
