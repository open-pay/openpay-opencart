...
class ControllerExtensionPaymentOpenpayPse extends Controller {
    ...
    public function index() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            $setting = $this->model_setting_setting->getSetting('payment_openpay_pse');
            $this->merge($setting, $this->request->post, true);
            
            $mode = $this->request->post['payment_openpay_pse_mode'] ? 'test' : 'live';            
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('extension/payment/openpay_pse', 'user_token=' . $this->session->data['user_token'], true),
        );

        $data['action'] = $this->url->link('extension/payment/openpay_pse', 'user_token=' . $this->session->data['user_token'], true);
        ...
        $data['payment_openpay_pse_mode'] = $this->fillSetting('payment_openpay_pse_mode');        
        $data['payment_openpay_pse_order_status_id'] = $this->fillSetting('payment_openpay_pse_order_status_id');
        $data['payment_openpay_pse_title'] = $this->fillSetting('payment_openpay_pse_title', $this->language->get('text_title'));
        $data['payment_openpay_pse_iva'] = $this->fillSetting('payment_openpay_pse_iva', 0);
        ...
        $data['payment_openpay_pse_status'] = $this->fillSetting('payment_openpay_pse_status');
        $data['payment_openpay_pse_total'] = $this->fillSetting('payment_openpay_pse_total');
        $data['payment_openpay_pse_sort_order'] = $this->fillSetting('payment_openpay_pse_sort_order');        
        $data['payment_openpay_pse_geo_zone_id'] = $this->fillSetting('payment_openpay_pse_geo_zone_id');

        $data['header'] = $this->load->controller('common/header');
        $data['column_left'] = $this->load->controller('common/column_left');
        $data['footer'] = $this->load->controller('common/footer');
        
        $this->response->setOutput($this->load->view('extension/payment/openpay_pse', $data));
    }

    protected function validate() {
        $min_total = 1;
        if (!$this->user->hasPermission('modify', 'extension/payment/openpay_pse')) {
        ...
        if ($this->request->post['payment_openpay_pse_mode']) {            
            ...
            if (empty($this->request->post['payment_openpay_pse_test_secret_key'])) {
                $this->error['test_secret_key'] = $this->language->get('error_test_secret_key');
            }            
            
            if(!$this->getMerchantInfo($this->request->post['payment_openpay_pse_test_merchant_id'], $this->request->post['payment_openpay_pse_test_secret_key'], $this->request->post['payment_openpay_pse_mode'])){
                $this->error['test_merchant_account'] = $this->language->get('error_test_merchant_account');                
            }
            
        } else {
            
            if (empty($this->request->post['payment_openpay_pse_live_merchant_id'])) {
                $this->error['live_merchant_id'] = $this->language->get('error_live_merchant_id');
            }
            
            if (empty($this->request->post['payment_openpay_pse_live_secret_key'])) {
                $this->error['live_secret_key'] = $this->language->get('error_live_secret_key');
            }            
            
            if(!$this->getMerchantInfo($this->request->post['payment_openpay_pse_live_merchant_id'], $this->request->post['payment_openpay_pse_live_secret_key'], $this->request->post['payment_openpay_pse_mode'])){
                $this->error['live_merchant_account'] = $this->language->get('error_live_merchant_account');
            }
            
        }

        if (!isset($this->request->post['payment_openpay_pse_total']) || (float) $this->request->post['payment_openpay_pse_total'] < (float) $min_total) {
    ...
    private function createWebhook($mode){        
        if(!$this->config->get('payment_openpay_pse_'.$mode.'_webhook')){                        
            $protocol = (isset($this->request->server['HTTPS']) && (($this->request->server['HTTPS'] == 'on') || ($this->request->server['HTTPS'] == '1'))) ? 'https://' : 'http://';            
            $webhook_data = array(
                'url' =>  $protocol.$_SERVER['HTTP_HOST'].'/index.php?route=extension/payment/openpay_pse/webhook',
    ...
    public function install() {
        ...
        $this->model_extension_payment_openpay_pse->install();                
    }

    public function uninstall() {
        //$this->load->model('extension/payment/openpay_pse');
        //$this->model_extension_payment_openpay_pse->uninstall();
    ...
    private function getMerchantInfo($id, $sk, $mode) {
        ...
        $live_url = "https://api.openpay.co/v1";
