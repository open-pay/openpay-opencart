...
class ControllerPaymentOpenpayStores extends OpenpayController {
    ...
    public function index() {
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('payment/openpay_stores', 'token=' . $this->session->data['token'], 'SSL'),
        );

        $data['action'] = $this->url->link('payment/openpay_stores', 'token=' . $this->session->data['token'], 'SSL');
        ...
        $this->response->setOutput($this->load->view('payment/openpay_stores.tpl', $data));
    }

    protected function validate() {
        if (!$this->user->hasPermission('modify', 'payment/openpay_stores')) {
    ...
    protected function createWebhook($mode){
        ...
        if(!$this->config->get('openpay_'.$mode.'_webhook')){
            $webhook_data = array(
                'url' => $this->config->get('config_ssl').'index.php?route=payment/openpay_stores/webhook',
    ...
    public function install() {
        ...
        $this->model_payment_openpay_stores->createOrderTable();
        $this->model_payment_openpay_stores->createTransactionTable();        
        $this->model_payment_openpay_stores->createCustomerTable();
    }

    public function uninstall() {
//        if ($this->config->get('openpay_uninstall_table')) {
//            $this->load->model('payment/openpay_stores');
//            $this->model_payment_openpay_stores->deleteOrderTable();
//            $this->model_payment_openpay_stores->deleteTransactionTable();
//            $this->model_payment_openpay_stores->deleteCustomerTable();
//        }
...
//        }
    ...
    public function orderAction() {
        if (defined('PRO_MOD') && PRO_MOD && isset($this->request->get['order_id']) && ( $charge = $this->fetchCharge($this->request->get['order_id']) )) {
            ...
            $data['txn'] = $this->model_payment_openpay_stores->getTransactions(array('charge_ref' => $charge->id));
            $data['url_capture'] = HTTPS_SERVER . 'index.php?route=payment/openpay_stores/jsonCapture&token=' . $this->session->data['token'] . '&order_id=' . $this->request->get['order_id'];
            $data['url_refund'] = HTTPS_SERVER . 'index.php?route=payment/openpay_stores/jsonRefund&token=' . $this->session->data['token'] . '&order_id=' . $this->request->get['order_id'];

            return $this->load->view('payment/openpay_stores_order.tpl', $data);
        }
    }
