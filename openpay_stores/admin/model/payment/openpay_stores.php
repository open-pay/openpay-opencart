...
class ModelPaymentOpenpayStores extends OpenpayModel {
    ...
    public function createPlanTable() {
        $this->db->query("CREATE TABLE IF NOT EXISTS " . DB_PREFIX . "openpay_plan (
				plan_id INT(11) UNSIGNED AUTO_INCREMENT,
				plan_ref VARCHAR(255),
				openpay_plan_id VARCHAR(255),
				date_added TIMESTAMP,
				amount INT(11),
				currency VARCHAR(3),
				`interval` VARCHAR(50),
    ...
    public function deletePlanTable() {
        $this->db->query("DROP TABLE IF EXISTS " . DB_PREFIX . "openpay_plan");
    }
